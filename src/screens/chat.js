...
class Chat extends Component {
    ...
    componentDidMount = async () => {
        // this.myTimer = setInterval(() => this.getLocation(), 60000);
        // this.myTimer2 = setInterval(() => this.updateLocation(), 60000);
    ...
    render() {
        ...
        const data = []
        chat.forEach((Listed, key) => {
            data[key] = users.find((item) => item.id === Listed.id)
        })
        return (
            <View>
                {this.state.isLoading == true ?
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => {
                            return (
                                <TouchableOpacity style={styles.button} activeOpacity={1} onPress={() => { this.props.navigation.navigate('ChatRoom', item) }}>
